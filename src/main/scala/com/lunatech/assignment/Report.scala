...
object Report {
   ...
   */
  val mapSurfaceType: Map[String, String] = runwaysDataList.map { x => (x.split(",")(2), x.split(",")(5)) }.toMap

  /**
   * A list with tuple2 containing country with runway's surface type utilizing mapRunwaysType.
   */
  val listCountryType: List[(String, String)] = airportDataList.map { x =>
   ...
   */
  val mostCommonRunways: List[String] = runwaysDataList.map(x => Try { x.split(",")(8) }.getOrElse("")).groupBy(identity)
